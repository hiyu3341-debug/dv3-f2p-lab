...
/* Ver.5.0 beta3: use compact thumbnails on dragon detail hero */
(function () {
  function applyDetailThumbnail() {
    const portrait = document.querySelector('.dragon-portrait');
    if (!portrait) return;

    const originalSrc = portrait.getAttribute('src');
    if (!originalSrc || originalSrc.startsWith('thumb-')) return;

    const cleanSrc = originalSrc.split('?')[0].split('#')[0];
    const fileName = cleanSrc.substring(cleanSrc.lastIndexOf('/') + 1);
    const stem = fileName.replace(/\.[^.]+$/, '');
    const thumbSrc = `thumb-${stem}.webp`;

    const probe = new Image();
    probe.onload = function () {
      portrait.src = thumbSrc;
      portrait.classList.add('dragon-portrait--thumb');
    };
    probe.onerror = function () {
      portrait.classList.add('dragon-portrait--fallback');
    };
    probe.src = thumbSrc;
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', applyDetailThumbnail);
  } else {
    applyDetailThumbnail();
  }
})();
