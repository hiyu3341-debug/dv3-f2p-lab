...
function renderDragons() {
  ...
  grid.innerHTML = filtered.map(d => `
    <article class="dragon-card">
      ...
      <div class="stats">
        <span>${d.star}<b>育成段階</b></span>
        <span>HP<b>${d.hp}</b></span>
        <span>攻撃<b>${d.atk}</b></span>
        <span>防御<b>${d.def}</b></span>
        <span>魔法<b>${d.mag}</b></span>
        <span>抵抗<b>${d.res}</b></span>
        <span>速度<b>${d.spd}</b></span>
      </div>
    </article>
  `).join('');
...
document.getElementById('calcPity').addEventListener('click', () => {
...
document.getElementById('calcGrowth').addEventListener('click', () => {
