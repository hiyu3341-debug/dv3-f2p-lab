(()=>{
  ...
  if(table){
    ...
    const sorted=core?core.ranked():[...dragons].sort((a,b)=>(b.total||0)-(a.total||0)||b.hp-a.hp||a.name.localeCompare(b.name,'ja'));
    body.innerHTML=sorted.map((d,i)=>`<tr><td>${d.totalRank||i+1}</td><td><a href="${esc(localUrl(d.url))}">${esc(d.name)}</a></td><td>${esc(d.rarity)}</td><td>${fmt(d.total)}</td><td>${fmt(d.hp)}</td><td>${fmt(d.atk)}</td><td>${fmt(d.def)}</td><td>${fmt(d.mag)}</td><td>${fmt(d.res)}</td><td>${fmt(d.spd)}</td></tr>`).join('');
  }
  ...
  const b=document.getElementById('menuButton'),n=document.getElementById('mainNav');if(b&&n)b.addEventListener('click',()=>{const open=n.classList.toggle('open');b.setAttribute('aria-expanded',String(open));});
})();
